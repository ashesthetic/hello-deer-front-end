...
const SalesReportPage: React.FC = () => {
	...
	const fetchDataWithParams = async (mode: ReportMode, year: number, month: number, startDate: string, endDate: string) => {
		...
		try {
			...
			// Fetch monthly trends data (for a broader range to show trends)
			// Calculate date range for last 12 months from the end date
	...
	const createMonthlyTrendChartData = (field: keyof MonthlyTrendData, label: string, color: string, isLiters: boolean = false) => {
		const data = monthlyTrends.map(month => Number(month[field]) || 0);
		const labels = monthlyTrends.map(month => month.month_name);
	...
	const monthlyFuelChartOptions = {
		...monthlyTrendChartOptions,
		plugins: {
			...monthlyTrendChartOptions.plugins,
			datalabels: {
				...monthlyTrendChartOptions.plugins.datalabels,
				formatter: function (value: any) {
					const numValue = parseFloat(value) || 0;
					return formatLiters(numValue);
				}
		...
		scales: {
			...
			y: {
				...
				ticks: {
					...monthlyTrendChartOptions.scales.y.ticks,
					callback: function (value: any) {
						const numValue = parseFloat(value) || 0;
						return formatLiters(numValue);
					}
						...
						{monthlyTrends.length > 0 && (
							<>
								<div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border border-blue-200 shadow-sm p-6">
									<h2 className="text-xl font-bold text-gray-900 mb-2">Monthly Trend</h2>
									<p className="text-sm text-gray-600 mb-4">
										Monthly aggregated performance across all key metrics (Last 12 months)
									</p>
								</div>
								...
								<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
									...
									<div className="bg-white rounded-lg shadow-lg p-6">
										...
										<div className="h-80">
											<Bar
												data={createMonthlyTrendChartData('total_sales', 'Total Sales', '#3B82F6')}
									...
									{/* Profits by Month */}
									<div className="bg-white rounded-lg shadow-lg p-6">
										<h3 className="text-lg font-semibold text-gray-900 mb-4">Profits by Month</h3>
										<div className="h-80">
											<Bar
												data={createMonthlyTrendChartData('total_profits', 'Total Profits', '#10B981')}
												options={monthlyTrendChartOptions}
											/>
										</div>
									</div>

									{/* Fuel Sales by Month (Liters) */}
									{monthlyTrends.some(m => m.fuel_quantity && m.fuel_quantity > 0) && (
										<div className="bg-white rounded-lg shadow-lg p-6">
											<h3 className="text-lg font-semibold text-gray-900 mb-4">Fuel Sales by Month (Liters)</h3>
											<div className="h-80">
												<Bar
													data={createMonthlyTrendChartData('fuel_quantity', 'Fuel Quantity (L)', '#F59E0B', true)}
													options={monthlyFuelChartOptions}
												/>
											</div>
										</div>
									)}
						...
						<div className="h-80">
							...
							/>
									...
									<div className="bg-white rounded-lg shadow-lg p-6">
										...
										<div className="h-80">
											<Bar
												data={createMonthlyTrendChartData('lottery', 'Lottery Sales', '#EF4444')}
												options={monthlyTrendChartOptions}
											/>
										</div>
									</div>
